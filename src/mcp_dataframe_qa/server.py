...
def build_server(qa: DataFrameQA) -> FastMCP:
    ...
    def ask_dataframe(question: str, dataset_id: str = "default") -> str:
        return (
            f"Use dataframe://{dataset_id}/profile for schema context. Then answer this question "
            f"with query_dataframe or execute_analysis_plan: {question}"
        )

    @mcp.prompt()
    def explain_dataframe(dataset_id: str = "default") -> str:
        return (
            f"Read dataframe://{dataset_id}/profile and summarize what analytical questions this "
